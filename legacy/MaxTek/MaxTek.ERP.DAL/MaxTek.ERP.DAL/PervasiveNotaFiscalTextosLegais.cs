...
internal class PervasiveNotaFiscalTextosLegais : INotaFiscalTextosLegais
{
	...
	private string ObterSql()
	{
		...
		stringBuilder.Append(" from " + BaseDadosERP.NomeBaseDados + "Erp.NFTextosLegais ");
	...
	public int InserirNotaFiscalTextosLegais(MapTableNotaFiscalTextosLegais NotaFiscalTextosLegais)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Insert into " + BaseDadosERP.NomeBaseDados + "Erp.NFTextosLegais (");
		stringBuilder.Append("ds_texto_legal) Values (?) ");
	...
	public int AlterarNotaFiscalTextosLegais(MapTableNotaFiscalTextosLegais NotaFiscalTextosLegais, Hashtable camposAlterados)
	{
		...
		if (camposAlterados.Count > 0)
		{
			IList<DbParameter> list = new List<DbParameter>();
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.Append("Update " + BaseDadosERP.NomeBaseDados + "Erp.NFTextosLegais Set ");
	...
	public int ExcluirNotaFiscalTextosLegais(int id)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Delete from " + BaseDadosERP.NomeBaseDados + "Erp.NFTextosLegais ");
		stringBuilder.Append("Where ");
		stringBuilder.Append(" id = ?");
		list.Add(BaseDadosERP.ObterNovoParametro("id", id));
