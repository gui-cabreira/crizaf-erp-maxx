...
internal class SQLServerNotaFiscalPaises : INotaFiscalPaises
{
	...
	private string ObterSql()
	{
		...
		stringBuilder.Append(" from [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Paises] ");
	...
	public MapTableNotaFiscalPaises ObterNotaFiscalPaises(int codigoPais)
	{
		...
		stringBuilder.Append(" [cd_pais] = @cd_pais");
	...
	public MapTableNotaFiscalPaises ObterNotaFiscalPaises(string pais)
	{
		...
		stringBuilder.Append(ObterSql());
		stringBuilder.Append("Where ");
		stringBuilder.Append(" [nm_pais] = @nm_pais");
	...
	public int InserirNotaFiscalPaises(MapTableNotaFiscalPaises NotaFiscalPaises)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Insert into [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Paises] (");
		stringBuilder.Append("[nm_pais], ");
		stringBuilder.Append("[cd_pais]) Values (@nm_pais,@cd_pais) ");
		list.Add(BaseDadosERP.ObterNovoParametro("@nm_pais", NotaFiscalPaises.pais));
	...
	public int AlterarNotaFiscalPaises(MapTableNotaFiscalPaises NotaFiscalPaises, Hashtable camposAlterados)
	{
		...
		if (camposAlterados.Count > 0)
		{
			IList<DbParameter> list = new List<DbParameter>();
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.Append("Update [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Paises] Set ");
	...
	public int ExcluirNotaFiscalPaises(string pais)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Delete from [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Paises] ");
		stringBuilder.Append("Where ");
		stringBuilder.Append(" [nm_pais] = @nm_pais");
		list.Add(BaseDadosERP.ObterNovoParametro("@nm_pais", pais));
