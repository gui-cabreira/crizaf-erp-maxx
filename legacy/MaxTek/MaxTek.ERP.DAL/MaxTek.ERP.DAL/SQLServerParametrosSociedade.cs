...
internal class SQLServerParametrosSociedade : IParametrosSociedade
{
	...
	private string ObterSql()
	{
		...
		stringBuilder.Append(" from [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[Parametros].[Sociedade] ");
	...
	public int InserirParametrosSociedade(MapTableParametrosSociedade ParametrosSociedade)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Insert into [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[Parametros].[Sociedade] (");
		...
		stringBuilder.Append("[im_logo]) Values (@id,@cd_cnpj,@nm_razao_social,@nm_fantasia,@nm_endereco,@nm_bairro,@id_cidade,@id_estado,@id_pais,@cd_postal,@nm_telefone,@cd_inscricao_estadual,@nm_email,@nm_site,@im_logo) ");
		list.Add(BaseDadosERP.ObterNovoParametro("@id", ParametrosSociedade.id));
		...
		list.Add(BaseDadosERP.ObterNovoParametro("@cd_inscricao_estadual", ParametrosSociedade.inscricaoEstadual));
	...
	public int AlterarParametrosSociedade(MapTableParametrosSociedade ParametrosSociedade, Hashtable camposAlterados)
	{
		...
		if (camposAlterados.Count > 0)
		{
			IList<DbParameter> list = new List<DbParameter>();
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.Append("Update [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[Parametros].[Sociedade] Set ");
			...
			list.Add(BaseDadosERP.AddSQLSmartUpdate(stringBuilder, "InscricaoEstadual", "cd_inscricao_estadual", ParametrosSociedade.inscricaoEstadual, camposAlterados));
	...
	public int ExcluirParametrosSociedade(int id)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Delete from [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[Parametros].[Sociedade] ");
		stringBuilder.Append("Where ");
		stringBuilder.Append(" [id] = @id");
		list.Add(BaseDadosERP.ObterNovoParametro("@id", id));
