...
internal class PervasiveDesenvolvimentoClasseCampos : IDesenvolvimentoClasseCampos
{
	...
	private string ObterSql()
	{
		...
		stringBuilder.Append("nm_classe_propriedad as nomePropriedade, ");
		stringBuilder.Append("ic_pk as isPk");
		stringBuilder.Append(" from " + BaseDadosERP.NomeBaseDados + "Erp.DevClasseCampos ");
	...
	public int InserirDesenvolvimentoClasseCampos(MapTableDesenvolvimentoClasseCampos DesenvolvimentoClasseCampos)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Insert into " + BaseDadosERP.NomeBaseDados + "Erp.DevClasseCampos (");
		...
		stringBuilder.Append("nm_classe_propriedad, ");
		stringBuilder.Append("ic_pk) Values (?,?,?,?,?,?,?,?,?,?,?) ");
		...
		list.Add(BaseDadosERP.ObterNovoParametro("nm_classe_propriedad", DesenvolvimentoClasseCampos.nomePropriedade));
	...
	public int AlterarDesenvolvimentoClasseCampos(MapTableDesenvolvimentoClasseCampos DesenvolvimentoClasseCampos, Hashtable camposAlterados)
	{
		...
		if (camposAlterados.Count > 0)
		{
			IList<DbParameter> list = new List<DbParameter>();
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.Append("Update " + BaseDadosERP.NomeBaseDados + "Erp.DevClasseCampos Set ");
			...
			list.Add(BaseDadosERP.AddSQLSmartUpdate(stringBuilder, "NomePropriedade", "nm_classe_propriedad", DesenvolvimentoClasseCampos.nomePropriedade, camposAlterados));
	...
	public int ExcluirDesenvolvimentoClasseCampos(int id)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Delete from " + BaseDadosERP.NomeBaseDados + "Erp.DevClasseCampos ");
		stringBuilder.Append("Where ");
		stringBuilder.Append(" id = ?");
		list.Add(BaseDadosERP.ObterNovoParametro("id", id));
