...
internal class SQLServerNotaFiscalCidades : INotaFiscalCidades
{
	...
	private string ObterSql()
	{
		...
		stringBuilder.Append(" from [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Cidades] ");
	...
	public MapTableNotaFiscalCidades ObterNotaFiscalCidades(string cidade)
	{
		...
		stringBuilder.Append(ObterSql());
		stringBuilder.Append("Where ");
		stringBuilder.Append(" [ds_cidade] = @ds_cidade");
	...
	public MapTableNotaFiscalCidades ObterNotaFiscalCidades(int codigoEstado, string cidade)
	{
		...
		stringBuilder.Append(ObterSql());
		stringBuilder.Append("Where ");
		stringBuilder.Append(" [cd_uf] = @cd_uf and ");
		stringBuilder.Append(" [ds_cidade] = @ds_cidade");
	...
	public int InserirNotaFiscalCidades(MapTableNotaFiscalCidades NotaFiscalCidades)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Insert into [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Cidades] (");
		stringBuilder.Append("[cd_uf], ");
		stringBuilder.Append("[ds_cidade]) Values (@cd_uf,@ds_cidade) ");
	...
	public int AlterarNotaFiscalCidades(MapTableNotaFiscalCidades NotaFiscalCidades, Hashtable camposAlterados)
	{
		...
		if (camposAlterados.Count > 0)
		{
			IList<DbParameter> list = new List<DbParameter>();
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.Append("Update [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Cidades] Set ");
	...
	public int ExcluirNotaFiscalCidades(int id)
	{
		IList<DbParameter> list = new List<DbParameter>();
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append("Delete from [" + BaseDadosERP.NomeServidor + "].[" + BaseDadosERP.NomeBaseDados + "].[NotaFiscal].[Cidades] ");
		stringBuilder.Append("Where ");
		stringBuilder.Append(" [id] = @id");
		list.Add(BaseDadosERP.ObterNovoParametro("@id", id));
