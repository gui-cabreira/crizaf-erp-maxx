...
public class rptEtiquetaFordMasterLabel : XtraReport
{
	private NotaFiscalNotasFiscaisItens _itemNotaFiscal;

	private decimal quantidadePecas;
	...
	public NotaFiscalNotasFiscaisItens ItemNotaFiscal
	{
		...
		set
		{
			...
			quantidadePecas = ItemNotaFiscal.Quantidade;
		}
	}
	...
	public decimal QuantidadePecas
	{
		set
		{
			quantidadePecas = value;
		}
	}

	public rptEtiquetaFordMasterLabel()
	{
		InitializeComponent();
	}
	...
	private void PageHeader_BeforePrint(object sender, PrintEventArgs e)
	{
		...
		((XRControl)txQuantidade).Text = quantidadePecas.ToString("F0");
		...
		if (ItemNotaFiscal.FichaExpedicaoItemRef != null && ItemNotaFiscal.FichaExpedicaoItemRef.EstoqueRef != null)
		{
			((XRControl)txPeso).Text = Math.Round(ItemNotaFiscal.FichaExpedicaoItemRef.EstoqueRef.PesoProduto * quantidadePecas, 2).ToString();
		}
	}
	...
	private void InitializeComponent()
	{
		...
		((XRControl)PageHeader).Controls.AddRange((XRControl[])(object)new XRControl[44]
		{
			(XRControl)xrLine12,
			(XRControl)xrLine11,
			(XRControl)xrLine10,
		...
		txNomeFantasia.AutoModule = true;
		...
		((XRControl)txNomeFantasia).SizeF = new SizeF(746.1248f, 148.3784f);
		...
		((XRControl)txCodigoVendedorEDI).LocationFloat = new PointFloat(945.5834f, 24.07379f);
		...
		((XRControl)xrLabel12).Font = new Font("Arial", 15.75f, FontStyle.Bold, GraphicsUnit.Point, 0);
		((XRControl)xrLabel12).LocationFloat = new PointFloat(1315.108f, 25.00001f);
		((XRControl)xrLabel12).Name = "xrLabel12";
		((XRControl)xrLabel12).Padding = new PaddingInfo(5, 5, 0, 0, 254f);
		((XRControl)xrLabel12).SizeF = new SizeF(466.8915f, 58.42f);
		((XRControl)xrLabel12).StylePriority.UseFont = false;
		((XRControl)xrLabel12).Text = "MASTER LABEL";
